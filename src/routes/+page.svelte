			...
			{#if project}
				<div class="mt-8 space-y-4">
                    ...
                    <div class="mb-4 flex flex-wrap gap-2">
                        <span class="inline-flex items-center rounded-full border border-rose-200 bg-rose-100 px-3 py-1 text-xs font-medium text-rose-900">
                            {project.difficulty}
                        </span>
                        <span class="inline-flex items-center rounded-full border border-blue-200 bg-blue-100 px-3 py-1 text-xs font-medium text-blue-900">
                            {project.domain}
                        </span>
                    </div>
                    
					<div class="text-[14px] leading-relaxed text-stone-800 border border-stone-200 rounded-xl p-4 bg-white">
                        <div class="font-semibold text-stone-900 mb-2">Project brief</div>
						{project.description}
					</div>
                    ...
                    <div class="text-[14px] leading-relaxed text-stone-800 border border-stone-200 rounded-xl p-4 bg-white">
						...
						{#each milestones as milestone}
							<div class="mb-2">
								<div class="font-medium text-stone-900 mb-1">{milestone.title}</div>
							</div>
						{/each}
					</div>
                    
                    <div class="text-[14px] leading-relaxed text-stone-800 border border-stone-200 rounded-xl p-4 bg-white">
                        <div class="font-semibold text-stone-900 mb-2">Skills you'll use/learn</div>
                        <div class="flex flex-wrap gap-2">
                            {#each project.skills as skill}
                                <span class="inline-flex items-center rounded-full border border-stone-200 bg-stone-100 px-3 py-1 text-[11px] font-medium text-stone-700">
                                    {skill}
                                </span>
                            {/each}
                        </div>
